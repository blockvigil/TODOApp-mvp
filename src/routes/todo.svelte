<script>
    ...
    async function postTodo(note) {
      return new Promise( async(resolve, reject) => {
        const contractResourcePath = evAPIPrefix + '/contract/' + contractAddress + '/addTodo';
        try{
    ...
    onDestroy(() => {
      unsubscribeWSTxStore();
    })
</script>

<svelte:head>
	<title>Live collaborative TODO</title>
</svelte:head>

<!-- Design and idea credits: https://freshman.tech/todo-list/ -->
